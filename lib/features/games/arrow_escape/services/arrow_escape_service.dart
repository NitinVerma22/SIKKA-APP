import 'dart:convert';
import 'package:flutter/foundation.dart';
import 'package:http/http.dart' as http;
import 'package:flutter_secure_storage/flutter_secure_storage.dart';
import 'package:shared_preferences/shared_preferences.dart';
import '../../../../core/auth/auth_service.dart';

class ArrowEscapeProgress {
  final int maxUnlockedLevel;
  final Map<int, int> starsMap;
  final int multiplier;

  ArrowEscapeProgress({
    required this.maxUnlockedLevel,
    required this.starsMap,
    required this.multiplier,
  });
}

class ArrowEscapeService {
  final FlutterSecureStorage _secureStorage = const FlutterSecureStorage();

  String get _arrowEscapeUrl => AuthService.baseUrl.replaceAll('/auth', '/v1/arrow-escape');

  Future<Map<String, String>> _getHeaders() async {
    final token = await _secureStorage.read(key: 'jwt_token');
    return {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ${token ?? ""}',
    };
  }

  Future<ArrowEscapeProgress> loadProgress() async {
    int maxLevel = 1;
    Map<int, int> stars = {};
    int multiplier = 2;

    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString('sikkaplay_arrow_escape_progress');
      if (raw != null) {
        final data = json.decode(raw);
        maxLevel = data['maxUnlockedLevel'] ?? 1;
        if (data['stars'] != null) {
          (data['stars'] as Map<String, dynamic>).forEach((k, v) {
            final lvl = int.tryParse(k);
            if (lvl != null) stars[lvl] = (v as num).toInt();
          });
        }
      }

      final headers = await _getHeaders();
      final res = await http.get(
        Uri.parse('$_arrowEscapeUrl/progress'),
        headers: headers,
      ).timeout(const Duration(seconds: 6));

      if (res.statusCode == 200) {
        final body = json.decode(res.body);
        if (body['success'] == true) {
          maxLevel = max(maxLevel, (body['maxUnlockedLevel'] ?? 1) as int);
          multiplier = (body['multiplier'] ?? 2) as int;
          if (body['stars'] != null) {
            (body['stars'] as Map<String, dynamic>).forEach((k, v) {
              final lvl = int.tryParse(k);
              if (lvl != null) stars[lvl] = (v as num).toInt();
            });
          }
          await saveLocalProgress(maxLevel, stars);
        }
      }
    } catch (e) {
      debugPrint('[ArrowEscapeService] Load progress fallback: $e');
    }

    return ArrowEscapeProgress(
      maxUnlockedLevel: maxLevel,
      starsMap: stars,
      multiplier: multiplier,
    );
  }

  Future<void> saveLocalProgress(int maxLevel, Map<int, int> stars) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final Map<String, dynamic> data = {
        'maxUnlockedLevel': maxLevel,
        'stars': stars.map((k, v) => MapEntry(k.toString(), v)),
      };
      await prefs.setString('sikkaplay_arrow_escape_progress', json.encode(data));
    } catch (e) {
      debugPrint('Error saving local arrow escape progress: $e');
    }
  }

  Future<Map<String, dynamic>> claimLevelReward({
    required int levelNumber,
    required int stars,
    required int score,
  }) async {
    try {
      final headers = await _getHeaders();
      final res = await http.post(
        Uri.parse('$_arrowEscapeUrl/complete-level'),
        headers: headers,
        body: json.encode({
          'levelNumber': levelNumber,
          'stars': stars,
          'score': score,
        }),
      ).timeout(const Duration(seconds: 8));

      if (res.statusCode == 200) {
        return json.decode(res.body) as Map<String, dynamic>;
      }
    } catch (e) {
      debugPrint('Error claiming arrow escape reward: $e');
    }

    return {
      'success': true,
      'coinsEarned': levelNumber * 2,
      'newUnlockedLevel': levelNumber + 1,
    };
  }

  int max(int a, int b) => a > b ? a : b;
}
